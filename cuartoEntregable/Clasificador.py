...
class Clasificador:
    ...
    def __init__(self):
        self.samp_rate = 256
        self.win_size = 256
        ytest, xtest, self.n_channels, = self.dataAnalisys("Izquierda - Derecha - Cerrado 2.txt")
        y, x, self.n_channels, = self.dataAnalisys("Izquierda - Derecha - Cerrado 1.txt")
        print("==============================LINEAL")
            ...
            try:
                ...
                if (elapsed_time > 0.1):
                    ...
                    if (contador > self.win_size):
                        ...
                        start_freq = next(
                            x for x, val in enumerate(freq) if val >= 4.0)
                        end_freq = next(
                            x for x, val in enumerate(freq) if val >= 60.0)
